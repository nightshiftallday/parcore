...
generate
    for (genvar I = 0; I < N_STREAMS; I++) begin : gen
        ...
        typed_ndata_i #(DATABEAT_SIZE) dec_out(clk, rst_n);
        ndata_i #(data8_t, DATABEAT_SIZE) dec_out_bytes(clk, rst_n);
        `DATA_ASSIGN(dec_out, dec_out_bytes)

        ndata_i #(data8_t, AXI_DATA_SIZE) dec_out_bytes_resized (clk, rst_n);
        ...
        NDataToAXI #(data8_t, DATABEAT_SIZE) inst_heap_to_axi (
            .clk(clk),
            .rst_n(rst_n),
