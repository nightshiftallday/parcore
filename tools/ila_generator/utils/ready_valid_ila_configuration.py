...
class ReadyValidILAConfiguration(ILAConfiguration):
    
    def __init__(self):
        super().__init__()
        self.name = ""
        self.array = None
        self.data_signal_width = 512
    ...
    def set_tdata_signal_width(self, width: int):
        self.data_signal_width = width

    def get_signal_map(self) -> Dict[str, int]:
        return {
            "valid": 1,
            "ready": 1,
            "data": self.data_signal_width
        }

    @staticmethod
    def try_parse(tokens: List[Token]) -> ILAConfiguration:
        configuration = ReadyValidILAConfiguration()
